...
async function main() {
  ...
  const { data: original } = await sb.from("narratives").select("status, reviewed_at, sent_at").eq("id", B2).single();
  ...
  console.log("unreview on sent (guard):", n.status, "— unchanged:", n.status === "sent");

  // stage a reviewed state, then unreview for real
  await sb.from("narratives").update({ status: "reviewed", sent_at: null }).eq("id", B2);
  await unreviewNarrative(B2);
  n = (await getNarrativeContext(B2))!.narrative;
  console.log("unreview on reviewed:", n.status, "· reviewedAt cleared:", !n.reviewedAt);
