...
async function main() {
  ...
  console.log(
    `1. same week    → brief "${brief.week.label}" vs narrative "${narrative.week.label}": ${brief.week.label === narrative.week.label}`,
  );
  console.log(
    `   same snapshot → ${brief.snapshotId === narrative.snapshotId}`,
  );
  console.log(
    `   headlines=${brief.headlines.length}, risk+win present: ${Boolean(brief.risk && brief.win)}`,
  );
  const h3 = brief.headlines[2];
  console.log(
    `   H3 refs → ${h3.evidenceRefs.map((r) => r.itemId).join(",")} (E6,E7 expected)`,
  );
  ...
  await updateLoomHeadline(
    h1.id,
    "CPO $26.40 — nine percent under target at higher spend",
  );
  const after = await getLoomBriefContext(B1);
  const editedH1 = after!.brief.headlines[0];
  const claimsAfter = (await getNarrativeContext(B1))!.narrative.claims
    .map((c) => c.text)
    .join("|");
  ...
  console.log(
    `   restored: ${(await getLoomBriefContext(B1))!.brief.headlines[0].text === originalText}`,
  );
