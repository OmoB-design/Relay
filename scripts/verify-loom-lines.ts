...
async function main() {
  ...
  await updateLoomLine(before.brief.id, "risk", "CPCs ran warm midweek — settled Friday, no action needed.");
  const after = (await getLoomBriefContext(B1))!;
  console.log("risk updated:", after.brief.risk !== originalRisk, "| win untouched:", after.brief.win === before.brief.win);
  await updateLoomLine(before.brief.id, "risk", originalRisk);
  console.log("restored:", (await getLoomBriefContext(B1))!.brief.risk === originalRisk);
}
