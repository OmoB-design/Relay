...
check("week is 7 days", format(addDays(parseISO(wk.start), 6), "yyyy-MM-dd"), wk.end);
check("yesterday is today - 1", yesterday(), format(addDays(now(), -1), "yyyy-MM-dd"));
check("week end is not after yesterday", parseISO(wk.end) <= parseISO(yesterday()), true);

console.log("\nWeekday preservation (a Thursday spike must stay a Thursday)");
for (const seed of [SEED_WEEK_START, "2026-07-09", SEED_WEEK_END, "2026-06-22"]) {
  check(
...
check("cross-month form", labelFor("2026-07-27", "2026-08-02"), "Jul 27 – Aug 2");
