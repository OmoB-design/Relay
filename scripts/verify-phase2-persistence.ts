...
async function main() {
  ...
  // --- KPI edit round-trip ---
  await updateKpi(cpo.id, {
    label: cpo.label,
    target: 27.5,
    polarity: cpo.polarity,
  });
  ...
  console.log(
    `3. restore   → target $${restored!.kpis.find((k) => k.id === cpo.id)!.target}`,
  );
  ...
  console.log(
    `4. add       → sensitivity {type: "${added.type}", text: "${added.text.slice(0, 30)}…"} (structured, typed)`,
  );
  ...
  console.log(
    `5. remove    → ${gone ? "deleted cleanly" : "STILL PRESENT (fail)"}`,
  );
