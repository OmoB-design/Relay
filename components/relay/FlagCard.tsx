...
/* FlagCard — Figma component set "Flag" (node 3:15460), 7 variants.
   ...
   Anatomy: one rounded card, then a tinted panel holding a header strip
   (client mark · name · category · delta) above the headline and diagnostic,
   with the actions in a footer outside the panel.

   FOUR PLACES THIS DEPARTS FROM THE FRAMES, deliberately:
   ...
   4. ONE hairline, not three. Figma nests three shells at radius 18 carrying
      1px / 0.8 / 0.7 borders; concentric hairlines at the same radius stack into
      banding rather than depth, so they resolve to a single 0.7px edge — which
      is what the frames actually read as.                                      */

type Mode = "idle" | "confirming" | "dismissed";

/* ONE shell. Figma nests three at radius 18 with 1px / 0.8 / 0.7 hairlines, but
   concentric borders at the same radius stack into banding rather than depth —
   the frames read as a single edge, so that is what this draws. */
const CARD =
  "overflow-hidden rounded-18 border-fig border-border bg-surface-primary shadow-card";
const PANEL =
  "flex flex-col gap-4 overflow-hidden rounded-14 border-fig border-border bg-panel py-2";
